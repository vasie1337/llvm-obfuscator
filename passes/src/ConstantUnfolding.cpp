...
static GlobalVariable *getOrCreateOpaqueZero(Module &M, unsigned BitWidth) {
    ...
    return new GlobalVariable(M, Ty, /*isConstant=*/false, GlobalValue::PrivateLinkage,
                              Constant::getNullValue(Ty), Name);
}

PreservedAnalyses ConstantUnfolding::run(Function &F, FunctionAnalysisManager &AM) {
    ...
    for (auto &T : Targets) {
        ...
        Value *OpaqueZ = Builder.CreateLoad(Ty, getZero(BW), /*isVolatile=*/true);
        ...
        case 0: {
            ...
            Value *Sum = Builder.CreateAdd(OpaqueZ, ConstantInt::get(Ty, Orig + K));
