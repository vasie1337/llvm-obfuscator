...
static Value *promoteToVector(BinaryOperator *BO, IRBuilder<> &Builder,
                              std::mt19937 &RNG) {
  ...
  auto RandConst = [&]() -> Value * {
    ...
    return ConstantInt::get(ScalarTy, ((uint64_t)RNG() << 32) | RNG());
  };
  ...
  auto BuildVec = [&](Value *Real, Value *Other) -> Value * {
    ...
    for (unsigned i = 0; i < NumElts; i++) {
      ...
      if (i == RealLane) {
        ...
        switch (RNG() % 5) {
        case 0:  Elt = Builder.CreateXor(Real, RandConst());  break;
        case 1:  Elt = Builder.CreateAdd(Real, RandConst());  break;
        case 2:  Elt = Builder.CreateSub(Other, RandConst()); break;
        case 3:  Elt = Builder.CreateOr(Real, RandConst());   break;
        default: Elt = Builder.CreateMul(Other, RandConst()); break;
        }
      }
  ...
  Value *VResult;
  switch (BO->getOpcode()) {
  case Instruction::Add: VResult = Builder.CreateAdd(VA, VB); break;
  case Instruction::Sub: VResult = Builder.CreateSub(VA, VB); break;
  case Instruction::Mul: VResult = Builder.CreateMul(VA, VB); break;
  case Instruction::Xor: VResult = Builder.CreateXor(VA, VB); break;
  case Instruction::And: VResult = Builder.CreateAnd(VA, VB); break;
  case Instruction::Or:  VResult = Builder.CreateOr(VA, VB);  break;
  default:
    return nullptr;
  }
...
PreservedAnalyses SIMDObfuscation::run(Function &F,
