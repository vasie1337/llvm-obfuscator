...
PreservedAnalyses ControlFlowFlattening::run(Function &F, FunctionAnalysisManager &AM) {
    ...
    std::mt19937 Rng(0x4f425343); // NOLINT(bugprone-random-generator-seed)
    ...
    AllocaInst *SwitchVar = EntryBuilder.CreateAlloca(EntryBuilder.getInt32Ty(), nullptr);
    ...
    BasicBlock *DispatchBB = BasicBlock::Create(F.getContext(), "cff.dispatch", &F, FirstBody);
    ...
    LoadInst *SwitchLoad = DispBuilder.CreateLoad(DispBuilder.getInt32Ty(), SwitchVar);
    BasicBlock *DefaultBB = BasicBlock::Create(F.getContext(), "cff.default", &F);
    SwitchInst *Dispatch = DispBuilder.CreateSwitch(SwitchLoad, DefaultBB, OrigBlocks.size());
    ...
    for (size_t I = 0; I < OrigBlocks.size(); ++I) {
        ...
        if (auto *Br = dyn_cast<BranchInst>(Term)) {
            ...
            if (Br->isUnconditional()) {
                ...
                int32_t FalseID = 0;
                for (size_t J = 0; J < OrigBlocks.size(); ++J) {
                    if (OrigBlocks[J] == TrueBB) {
                        TrueID = BlockIDs[J];
                    }
                    if (OrigBlocks[J] == FalseBB) {
                        FalseID = BlockIDs[J];
                    }
                }

                Value *SelID =
                    Builder.CreateSelect(Cond, Builder.getInt32(TrueID), Builder.getInt32(FalseID));
            ...
            for (auto Case : SI->cases()) {
                ...
                int32_t CaseID = 0;
                for (size_t J = 0; J < OrigBlocks.size(); ++J) {
                    if (OrigBlocks[J] == CaseDest) {
                        CaseID = BlockIDs[J];
                        break;
                    }
                }

                Value *Cmp = Builder.CreateICmpEQ(SI->getCondition(), Case.getCaseValue());
